...
> ⚠️ **This property is deprecated and will be removed in the SDK 43. Please, implement it on your own using `expo-application`'s `androidId` on Android and a storage API such as `expo-secure-store` on iOS and `localStorage` on Web.**
